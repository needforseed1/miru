...
| Qt 6.5+ Base | Core, GUI, networking, SQL, Qt Test |
...
The current suite covers release badge matching, AIOStreams stream filtering,
metadata normalization, settings persistence and file permissions, mpv launch
arguments, and local watch-history resume behavior. QML lint targets are also
generated by Qt, for example:

```bash
cmake --build build --target miru_qmllint
```
...
src/player/     External mpv launcher and argument helpers
qml/            Qt Quick UI
